...
Model names may contain `/`. Pass those slashes as path separators when calling the detail endpoint, for example `/v1/models/openai/gpt-5.4`; the server route consumes the remaining path as one logical identifier. Discover the current ID from `GET /v1/models` instead of copying an identifier from an older guide.
