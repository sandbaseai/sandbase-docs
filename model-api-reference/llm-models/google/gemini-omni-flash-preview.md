...
## Google-native video generation

For video generation, use the [Google Gemini Interactions API](/api-reference/gemini-interactions) with
`google/gemini-omni-flash-preview`. Submit the Interaction asynchronously with `background: true` and poll the returned
Interaction ID until it reaches a terminal status. The standard model API above remains available for Chat Completions
requests.
